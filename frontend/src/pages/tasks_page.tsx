...
import EmptyTasksRow from "../components/task/empty_tasks_row";
...
export default function TasksPage() {
  ...
  const [sort, setSort] = useState<"deadline_asc" | "deadline_desc">("deadline_asc");
    ...
    <div className="relative mx-auto max-w-6xl p-4 sm:p-6">
      <div className="mb-4 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        ...
        <div className="flex flex-col gap-2 sm:flex-row sm:items-center">
          <select
            value={sort}
            onChange={(e) => setSort(e.target.value as "deadline_asc" | "deadline_desc")}
      ...
      <div className="app-card relative overflow-hidden">
        {loading ? (
          <div className="px-4 py-8 text-center text-(--color-muted)">Đang tải...</div>
        ) : tasks.length === 0 ? (
          <EmptyTasksRow />
        ) : (
                ...
                {tasks.map((task) => (
                  <tr key={task.taskId} className="border-b border-gray-100">
                    <td className="px-4 py-3">
                      <TaskStatusSelect
                        status={task.status}
                        onChange={(status) => handleStatusChange(task.taskId, status)}
                      />
                    </td>
